...
function GetSKUsFromQuesionnaires_not_NTZ_m(search) {
...
function CalculateTotal(single) {
	var q = new Query(
			"SELECT COUNT(U1.Question) FROM USR_SKUQuestions U1 WHERE U1.Single=@single "
					+ " AND (ParentQuestion=@emptyRef OR ParentQuestion IN (SELECT Question FROM USR_SKUQuestions "
...
function CalculateQty(single) {
	var q = new Query(
			"SELECT COUNT(U1.Answer) AS Answered "
...
function AddFilter(filterString, filterName, condition, connector) {
	...
	var q = new Query(
			"SELECT F.Id FROM USR_Filters F WHERE F.FilterType = @filterName");
	...
	if (res != null) {
		...
		filterString += condition
				+ " IN(SELECT F.Id FROM USR_Filters F WHERE F.FilterType = '"
				+ filterName + "') " + connector;
...
function GoToQuestionAction(control, answerType, question, sku, editControl,
	...
	if ((answerType).ToString() == (DB.Current.Constant.DataType.ValueList)
			.ToString()) {
		var q = new Query();
		q.Text = "SELECT Value, Value FROM Catalog_Question_ValueList WHERE Ref=@ref";
		q.AddParameter("ref", question);
		// Dialogs.DoChoose(q.Execute(), question, null, editControl,
		// DialogCallBack);
		DoChoose(q.Execute(), question, null, editControl, DialogCallBack,
				title);
	}

	if ((answerType).ToString() == (DB.Current.Constant.DataType.Snapshot)
			.ToString()) {
		var controlText;
		if (editControl.Text == "—")
			controlText = null;
		...
		AddSnapshot($.workflow.visit, null, GalleryCallBack, listChoice,
				"document.visit", title);
	}

	if ((answerType).ToString() == (DB.Current.Constant.DataType.DateTime)
			.ToString()) {
	...
	if ((answerType).ToString() == (DB.Current.Constant.DataType.Boolean)
			.ToString()) {
	...
	if (((answerType).ToString() == (DB.Current.Constant.DataType.String)
			.ToString())
			|| ((answerType).ToString() == (DB.Current.Constant.DataType.Integer)
					.ToString())
			|| ((answerType).ToString() == (DB.Current.Constant.DataType.Decimal)
					.ToString())) {
...
function AssignAnswer(control, question, sku, answer) {
	...
	var q = new Query(
			"UPDATE USR_SKUQuestions SET Answer="
					+ answerString
					+ ", AnswerDate=DATETIME('now', 'localtime') WHERE Question=@question AND SKU=@sku");
...
function AddSnapshot(objectRef, valueRef, func, listChoice, objectType, title) {
	...
	Dialog.Choose(title, listChoice, AddSnapshotHandler, [ objectRef, func,
			valueRef, objectType ]);
}
...
function DoChoose(listChoice, entity, attribute, control, func, title) {
	...
	Dialog.Choose(title, listChoice, startKey, func, [ entity, attribute,
			control ]);
}
